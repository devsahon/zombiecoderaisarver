...
export class BackupSystem {
  ...
  private getFileList(dirPath: string): string[] {
    ...
    const scanDirectory = (path: string, relativePath: string = '') => {
  ...
  private async createZipArchive(sourcePath: string, zipPath: string) {
    ...
    const tempZipPath = path.join(this.backupDir, 'temp_backup');
    if (fs.existsSync(tempZipPath)) {
      fs.rmSync(tempZipPath, { recursive: true });
    }
    this.copyDirectory(sourcePath, tempZipPath);
  }
